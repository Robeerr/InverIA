    ...
    <div className="card-flat p-4">
      ...
      <div className="flex gap-2">
        <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://youtu.be/..."
          className="flex-1 px-3 py-2 rounded-iv border border-linea bg-white text-tinta text-sm min-w-0" />
        <button onClick={go} disabled={busy} className="px-4 py-2 rounded-iv bg-marca text-white text-sm font-semibold disabled:opacity-50 shrink-0">
      ...
      {manual && (
        <div className="mt-2 space-y-2">
          <input value={fuente} onChange={(e) => setFuente(e.target.value)} placeholder="Fuente (ej. Vídeo JaviZone) — opcional"
            className="w-full px-3 py-2 rounded-iv border border-linea bg-white text-tinta text-sm" />
          <textarea value={texto} onChange={(e) => setTexto(e.target.value)} rows={5} placeholder="Pega aquí la transcripción del vídeo, o cualquier texto (artículo, análisis)…"
            className="w-full px-3 py-2 rounded-iv border border-linea bg-white text-tinta text-sm" />
          <button onClick={goText} disabled={busy} className="w-full py-2 rounded-iv bg-marca text-white text-sm font-semibold disabled:opacity-50">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <Brain size={24} weight="fill" className="text-marca shrink-0" />
          <div className="min-w-0">
            <h1 className="font-heading font-bold text-lg text-tinta leading-tight">Cerebro</h1>
            <p className="text-etiqueta text-tinta-3">Lo que InverIA aprende de tus fuentes (Telegram + newsletters).</p>
          </div>
        </div>
        <button onClick={load} className="shrink-0 p-2 rounded-iv border border-linea text-marca" title="Recargar">
      ...
      <div className="flex gap-1.5">
        {[["actividad", "Actividad"], ["conocimiento", "Conocimiento"], ["fuentes", "Fuentes"]].map(([k, l]) => (
          <button key={k} onClick={() => setTab(k)}
            className={`px-3 py-1 rounded-full text-xs font-semibold border ${tab === k ? "bg-marca text-white border-marca" : "border-linea text-tinta-3"}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="card-flat p-8 text-center text-tinta-3 text-sm">Cargando…</div>
      ) : tab === "actividad" ? (
        <div className="space-y-1.5">
          ...
          {actividad.length === 0 && <div className="card-flat p-6 text-center text-xs text-tinta-3">Aún no ha capturado nada. Cuando llegue algo a tus fuentes, aparecerá aquí.</div>}
          {actividad.map((a, i) => <ActivityItem key={i} a={a} />)}
        </div>
      ) : tab === "conocimiento" ? (
        <div className="space-y-2">
          <div className="flex gap-1.5 flex-wrap">
            <button onClick={() => setCat(null)} className={`px-2.5 py-1 rounded-full text-etiqueta border ${!cat ? "bg-marca text-white border-marca" : "border-linea text-tinta-3"}`}>Todo</button>
            {Object.entries(cats).map(([c, n]) => (
              <button key={c} onClick={() => setCat(c)} className={`px-2.5 py-1 rounded-full text-etiqueta border ${cat === c ? "bg-marca text-white border-marca" : "border-linea text-tinta-3"}`}>{c} · {n}</button>
            ))}
          </div>
          <div className="space-y-1.5">
            {filtered.map((p, i) => <Principio key={i} p={p} />)}
            {filtered.length === 0 && <div className="card-flat p-6 text-center text-xs text-tinta-3">Sin principios todavía.</div>}
          </div>
        </div>
      ) : (
        <div className="space-y-1.5">
          ...
          {fuentes.length === 0 && <div className="card-flat p-6 text-center text-xs text-tinta-3">Sin fuentes activas todavía.</div>}
        </div>
      )}
    </div>
  );
