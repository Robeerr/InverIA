...
function YouTubeBox({ onDone }) {
  ...
  return (
    <div className="card-flat p-4">
      ...
      {busy && <p className="text-etiqueta text-[#5c6b66] mt-2">Procesando y extrayendo ideas…</p>}

      <button onClick={() => setManual((v) => !v)} className="text-etiqueta text-[#5c6b66] underline mt-2">
      ...
      {res?.ok && (
        <div className="mt-3 border-l-2 border-[#4a7c59] pl-3 py-1">
          ...
          {res.acciones === 0 && res.aprendidos === 0 && (
            <p className="text-etiqueta text-[#5c6b66] mt-1">Leyó el vídeo ({res.chars} caracteres) pero no encontró picks ni método reutilizable.</p>
          )}
        </div>
      )}
      {res && !res.ok && <p className="text-etiqueta text-[#d85c41] mt-2">{res.error}</p>}
    </div>
  );
}
...
function Stat({ label, value, color }) {
  return (
    <div className="card-flat p-4 text-center">
      ...
      <p className="text-etiqueta uppercase tracking-[0.15em] text-[#5c6b66] font-mono mt-1">{label}</p>
    </div>
  );
}
    ...
    <div className="card-flat px-4 py-2.5">
      <button onClick={() => canOpen && setOpen((v) => !v)} className={`w-full flex items-start gap-2.5 text-left ${canOpen ? "cursor-pointer" : "cursor-default"}`}>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <p className="text-etiqueta font-semibold text-[#1a3a32] truncate">{a.fuente}</p>
            <span className="text-etiqueta text-[#8a958f] shrink-0 font-mono">{fmtFecha(a.at)}</span>
          </div>
          <p className={`text-apoyo text-[#5c6b66] leading-snug mt-0.5 ${open ? "" : "line-clamp-2"}`}>{a.snippet || "(sin texto)"}</p>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          {a.aprendidos > 0 && (
            <span className="text-etiqueta font-bold text-[#4a7c59] bg-[#4a7c5915] px-1.5 py-0.5 rounded-full">+{a.aprendidos}</span>
          )}
      ...
      {open && (
        <div className="mt-2 pl-6 space-y-1.5 border-t border-[#e5e0d8] pt-2">
          <p className="text-etiqueta uppercase tracking-wider text-[#5c6b66] font-mono">Aprendió:</p>
          {principios.map((p, i) => (
            <p key={i} className="text-apoyo text-[#0e1f1a] leading-snug flex gap-1.5">
    ...
    <div className="card-flat px-4 py-2.5">
      <button onClick={() => setOpen((v) => !v)} className="w-full flex items-start justify-between gap-2 text-left">
        <div className="min-w-0">
          <span className="text-etiqueta uppercase tracking-wider font-mono text-[#1a3a32] bg-[#e8efe9] px-1.5 py-0.5 rounded">{p.categoria}</span>
          <p className="text-sm text-[#0e1f1a] leading-snug mt-1">{p.principio}</p>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          {p.refuerzos > 1 && <span className="text-etiqueta font-mono font-bold text-[#4a7c59]">×{p.refuerzos}</span>}
          {p.detalle && <CaretDown size={13} className={`text-[#5c6b66] transition-transform ${open ? "rotate-180" : ""}`} />}
        </div>
      </button>
      {open && p.detalle && <p className="text-apoyo text-[#5c6b66] leading-relaxed mt-2">{p.detalle}</p>}
      {open && p.fuentes?.length > 0 && (
        <p className="text-etiqueta text-[#8a958f] mt-1.5 truncate">Fuentes: {p.fuentes.join(" · ")}</p>
      )}
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-1.5">
          <p className="text-etiqueta text-[#5c6b66] px-1">Cada mensaje/audio/foto que capta, en orden. Lo que aporta método suma principios; el ruido pasa sin añadir nada.</p>
        ...
        <div className="space-y-2">
          <div className="flex gap-1.5 flex-wrap">
            <button onClick={() => setCat(null)} className={`px-2.5 py-1 rounded-full text-etiqueta border ${!cat ? "bg-[#1a3a32] text-white border-[#1a3a32]" : "border-[#e5e0d8] text-[#5c6b66]"}`}>Todo</button>
            {Object.entries(cats).map(([c, n]) => (
              <button key={c} onClick={() => setCat(c)} className={`px-2.5 py-1 rounded-full text-etiqueta border ${cat === c ? "bg-[#1a3a32] text-white border-[#1a3a32]" : "border-[#e5e0d8] text-[#5c6b66]"}`}>{c} · {n}</button>
            ))}
          </div>
          <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          <p className="text-etiqueta text-[#5c6b66] px-1">De dónde viene el conocimiento y cuánto aporta cada fuente.</p>
          {fuentes.map((f, i) => (
            <div key={i} className="card-flat px-4 py-2.5 flex items-center justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-etiqueta text-[#5c6b66]">Última: {fmtFecha(f.ultima)}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-sm font-mono font-semibold text-[#4a7c59]">+{f.aprendidos}</p>
                <p className="text-etiqueta text-[#5c6b66]">{f.capturas} capturas</p>
              </div>
            </div>
          ))}
