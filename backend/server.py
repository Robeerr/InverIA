...
class TransactionCreate(BaseModel):
    ...
    broker: Optional[str] = "DEGIRO"


class TransactionsBulk(BaseModel):
...
async def get_portfolio():
    """Returns full portfolio: summary, open positions with live prices, trade history, realized P&L."""
    tx_docs = await db.portfolio_transactions.find({}, {"_id": 0}).to_list(2000)
    if not tx_docs:
        return {
            "summary": {
    ...
    computed = portfolio.compute_positions(tx_docs)
    hydrated = portfolio.hydrate_with_live_prices(computed)
    hydrated["transactions_count"] = len(tx_docs)
    return hydrated


@api_router.get("/portfolio/transactions")
async def list_transactions():
    items = await db.portfolio_transactions.find({}, {"_id": 0}).sort("date", -1).to_list(2000)
    return items


@api_router.post("/portfolio/transactions")
async def add_transactions(payload: TransactionsBulk):
    docs = []
    for tx in payload.transactions:
        action = tx.action.upper().strip()
        if action not in ("BUY", "SELL"):
            continue
        d = tx.model_dump()
        d["id"] = str(uuid.uuid4())
        d["action"] = action
        d["symbol"] = tx.symbol.upper().strip()
        d["created_at"] = datetime.now(timezone.utc).isoformat()
        docs.append(d)
    if docs:
        await db.portfolio_transactions.insert_many(docs)
    return {"inserted": len(docs)}


@api_router.delete("/portfolio/transactions/{tx_id}")
async def remove_transaction(tx_id: str):
    res = await db.portfolio_transactions.delete_one({"id": tx_id})
    if res.deleted_count == 0:
...
async def upload_portfolio_pdf(file: UploadFile = File(...)):
    """Parse a broker PDF and return extracted transactions + cash events for user review."""
    content = await file.read()
    if len(content) > 10 * 1024 * 1024:
        raise HTTPException(413, "Archivo demasiado grande (máximo 10MB)")
    try:
        # Hard cap below Render's 100s proxy timeout so we return a clean JSON error
        # with CORS headers instead of a bare 502 Bad Gateway from the proxy.
        parsed = await asyncio.wait_for(portfolio.parse_pdf_with_ai(content), timeout=90)
    except asyncio.TimeoutError:
        logger.warning("PDF parse exceeded 90s")
        raise HTTPException(
            504,
            "El PDF es demasiado grande o el servicio de IA está lento. "
            "Prueba a subir un PDF más pequeño (menos páginas) o vuelve a intentarlo en unos minutos.",
        )
    except Exception as e:
        logger.exception("PDF parse failed")
        raise HTTPException(500, f"Error procesando PDF: {str(e)[:200]}")
    return {
        "transactions": parsed.get("transactions", []),
        "cash_events": parsed.get("cash_events", []),
        "count": len(parsed.get("transactions", [])),
        "events_count": len(parsed.get("cash_events", [])),
        "warning": parsed.get("warning"),
        "debug": parsed.get("debug"),
    }


@api_router.post("/portfolio/cash-events")
async def add_cash_events(events: dict):
    ...
    for ev in items:
        ...
        docs.append(d)
    if docs:
        await db.portfolio_cash_events.insert_many(docs)
    return {"inserted": len(docs)}


@api_router.get("/portfolio/cash-events")
async def list_cash_events():
    events = await db.portfolio_cash_events.find({}, {"_id": 0}).sort("date", -1).to_list(5000)
    summary = portfolio.aggregate_cash_events(events)
    return {"events": events, **summary}


@api_router.delete("/portfolio/cash-events")
async def clear_cash_events():
    res = await db.portfolio_cash_events.delete_many({})
    return {"deleted": res.deleted_count}


@api_router.delete("/portfolio/transactions")
async def clear_all_transactions():
    res = await db.portfolio_transactions.delete_many({})
    # also clear cash events
    await db.portfolio_cash_events.delete_many({})
    return {"deleted": res.deleted_count}
